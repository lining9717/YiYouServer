...
7. **获取用户订单信息接口**
   ...
   |    code    |   int    |              若为1，代表成功；<br/>否则为不成功              |
   |  message   |  String  |                      返回的状态信息说明                      |
   |    data    | 自定义类 | 若无订单，data项为空，类型为String；<br/>否则具体返回类型见上图；<br/>status有四种状态:idle(闲置)、accepted(已被向导接单)、<br/>begin(用户选择了向导开始进行)、finished(已完成)；<br/>note代表用户对订单的备注 |
...
8. **向导获得订单信息接口**
   ...
   |    code    |   int    |              若为1，代表成功；<br/>否则为不成功              |
   |  message   |  String  |                      返回的状态信息说明                      |
   |    data    | 自定义类 | 若无订单，data项为空，类型为String；<br/>否则具体返回类型见上图 |
...
15. **用户获取收藏向导信息接口**
    ...
    |    code    |   int    |              若为1，代表成功；<br/>否则为不成功              |
    |  message   |  String  |                      返回的状态信息说明                      |
    |    data    | 自定义类 | 若无收藏想到，data项为空，类型为String；<br/>否则具体返回类型见上图 |
